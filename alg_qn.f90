SUBROUTINE alg_qn(fname,fvec,x,n,QTmat,Rmat,intlj,reevalj,check,rstit0,MaxLns,MAXITS,stpmax,TOLF)


use params, only: prec
USE alexutils, ONLY: get_diag,lower_triangle, outerprod,put_diag,unit_matrix
use alexfuncs, ONLY: qrdcmp,qrupdt,rsolv,fdjac

IMPLICIT NONE

INTEGER :: i,its,k,n,rstit,rstit0,MaxLns,MAXITS
REAL(prec) :: TOLF,TOLX,f,fold,stpmax,vabs,stpmx
REAL(prec), DIMENSION(n) :: x,fvec,c,d,fvcold,g,p,s,t,w,xold,diagJ
REAL(prec), PARAMETER :: EPS=epsilon(x)
REAL(prec), DIMENSION(n,n) :: Rmat,QTmat,rold,qtold
LOGICAL :: check,restrt,sing,intlj,reevalj
external fname
