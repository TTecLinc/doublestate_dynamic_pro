SUBROUTINE alg_gsqn(fname,fvec,x,n,QTmat,Rmat,check,rstit,MaxLns,MAXITS,updt,stpmax,TOLF)

! so far this is based on standard broyden without separate QR update

use params, only: prec
use paramsalg, only: tt
use paramsimsl
use alexutils, only: get_diag

IMPLICIT NONE

INTEGER :: i,j,its,k,l,n,rstit,nsmall,MaxLns,MaxIts
REAL(prec) :: TOLF,TOLX,f,fold,stpmax,vabs
REAL(prec), DIMENSION(n) :: x,fvec,fvcold,xold,p,g
real(prec),dimension(tt,n/tt)::p0,g0,f0
real(prec),dimension(n/tt,1)::s,w
real(prec),dimension(1,n/tt)::sr
REAL(prec), PARAMETER :: EPS=epsilon(x)
REAL(prec), DIMENSION(n/tt,n/tt) :: Rmat,QTmat,B,Bold,invB,transB
real(prec),dimension(n/tt)::diaginvB
real(prec)::srs
integer::nvec(n/tt)
...
B = matmul(QTmat,Rmat)							! Jacobi matrix => Broyden matrix B
nsmall=n/tt
...
REAL(prec) :: fold,stpmax,f,vabs
LOGICAL :: check
REAL(prec), PARAMETER :: ALF=1.0e-4,TOLX=epsilon(x)
REAL(prec) :: a,alam,alam2,alamin,b,disc,f2,pabs,rhs1,rhs2,slope,tmplam
external fname
