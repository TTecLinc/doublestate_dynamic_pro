...
REAL(prec), PARAMETER :: EPS=1.0e-4
INTEGER :: j,n
...
USE alexutils, ONLY: outerprod
IMPLICIT NONE
REAL(prec), DIMENSION(:,:), INTENT(INOUT) :: a
REAL(prec), DIMENSION(:), INTENT(OUT) :: c,d
real(prec)::vabs
LOGICAL, INTENT(OUT) :: sing
INTEGER:: k,n
REAL(prec) :: scale,sigma
...
USE alexutils, ONLY : ifirstloc

IMPLICIT NONE
REAL(prec), DIMENSION(:,:), INTENT(INOUT) :: r,qt
...
IMPLICIT NONE
REAL(prec), DIMENSION(:,:), TARGET, INTENT(INOUT) :: r,qt
INTEGER, INTENT(IN) :: i
REAL(prec), INTENT(IN) :: a,b
REAL(prec), DIMENSION(size(r,1)) :: temp
INTEGER:: n
REAL(prec) :: c,fact,s
...
IMPLICIT NONE
REAL(prec), DIMENSION(:,:), INTENT(IN) :: a
REAL(prec), DIMENSION(:), INTENT(IN) :: d
